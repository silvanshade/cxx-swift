...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["swift", "clang_importer", "serialized_swift_name", "owning_iterator"];
    let path_descendants = vec![];
    let cxx_include = "cxx-swift-abi/cxx/include/swift/ClangImporter/SerializedSwiftName/OwningIterator.hxx";
    let cxx_namespace = "cxx_swift::swift::clang_importer::serialized_swift_name::owning_iterator";
    let cxx_name = "SerializedSwiftNameOwningIterator";
    let rust_name = "SerializedSwiftNameOwningIterator";
