...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["swift", "ast", "ast_context"];
    let path_descendants = vec![];
    let cxx_include = "cxx-swift-abi/cxx/include/swift/AST/ASTContext.hxx";
    let cxx_namespace = "cxx_swift::swift::ast::ast_context";
    let cxx_name = "ASTContext";
    let rust_name = "AstContext";
    let lifetimes = ::cxx_memory_abi::indexmap::IndexMap::from_iter([("cfg", vec![])].into_iter());
