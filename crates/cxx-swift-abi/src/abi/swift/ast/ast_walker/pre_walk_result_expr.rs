...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["swift", "ast", "ast_walker", "pre_walk_result_expr"];
    let path_descendants = vec![];
    let cxx_include = "cxx-swift-abi/cxx/include/swift/AST/ASTWalker/PreWalkResultExpr.hxx";
    let cxx_namespace = "cxx_swift::swift::ast::ast_walker::pre_walk_result_expr";
    let cxx_name = "PreWalkResultExpr";
    let rust_name = "PreWalkResultExpr";
    ...
    let is_rust_copy_new = false;
    let is_rust_move_new = false;
