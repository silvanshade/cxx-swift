...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["swift", "ast", "ast_walker_rust"];
    let path_descendants = vec![];
    let cxx_include = "cxx-swift/cxx/include/swift/AST/ASTWalkerRust.hxx";
    let cxx_namespace = "cxx_swift::swift::ast::ast_walker_rust";
    let cxx_name = "ASTWalkerRust";
    let rust_name = "AstWalkerRust";
    let lifetimes =
        ::cxx_memory_abi::indexmap::IndexMap::from_iter([("state", vec![]), ("ctx", vec!["state"])].into_iter());
