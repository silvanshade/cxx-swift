...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["swift", "ast", "pattern"];
    let path_descendants = vec![];
    let cxx_include = "cxx-swift-abi/cxx/include/swift/AST/Pattern.hxx";
    let cxx_namespace = "cxx_swift::swift::ast::pattern";
    let cxx_name = "Pattern";
