...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["swift", "ast", "module_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-swift-abi/cxx/include/swift/AST/ModuleDecl.hxx";
    let cxx_namespace = "cxx_swift::swift::ast::module_decl";
    let cxx_name = "ModuleDecl";
    let rust_name = "ModuleDecl";
    ...
    let is_rust_drop = false && (self::ffi::cxx_is_destructible() && !self::ffi::cxx_is_trivially_destructible());
