...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["swift", "ast", "diagnostic_engine"];
    let path_descendants = vec![];
    let cxx_include = "cxx-swift-abi/cxx/include/swift/AST/DiagnosticEngine.hxx";
    let cxx_namespace = "cxx_swift::swift::ast::diagnostic_engine";
    let cxx_name = "DiagnosticEngine";
    let rust_name = "DiagnosticEngine";
    let lifetimes = ::cxx_memory_abi::indexmap::IndexMap::from_iter([("source_manager", vec![])].into_iter());
